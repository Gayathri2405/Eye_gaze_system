...
const Key5 = () => {
 ...
 // const [averageValue, setAverageValue] = useState(0);
  ...
  const firstBlockKeys = "GHI".split("");
  const secondBlockKeys = "JKLM".split("");
  ...
  useEffect(() => {
    ...
    const onResults = (results) => {
      ...
      if (results.multiFaceLandmarks && results.multiFaceLandmarks[0]) {
       ...
       if (positionIndicator === 'Left' && currentRoute !== '/key8') {
          const timer = setTimeout(() => {
            navigate('/key8');
          }, 2000);
          setGazingLeftTimer(timer);
        } else if (positionIndicator === 'Right' && currentRoute !== '/key9') {
          const timer = setTimeout(() => {
            navigate('/key9');
          }, 2000);
