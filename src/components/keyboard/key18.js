...
const Key18 = () => {
 ...
 // const [averageValue, setAverageValue] = useState(0);
  ...
  const firstBlockKeys = "".split("");
  const secondBlockKeys = "NO".split("");
      ...
      if (results.multiFaceLandmarks && results.multiFaceLandmarks[0]) {
        ...
        if (positionIndicator === 'Left' && currentRoute !== '/browser') {
          const timer = setTimeout(() => {
            navigate('/browser', { state: { letter: '' } });
        
          }, 2000);
          setGazingLeftTimer(timer);
        } else if (positionIndicator === 'Right' && currentRoute !== '/key22') {
          const timer = setTimeout(() => {
            navigate('/key22');
          }, 2000);
