...
const Key9= () => {
 ...
 // const [averageValue, setAverageValue] = useState(0);
  ...
  const firstBlockKeys = "JK".split("");
  const secondBlockKeys = "LM".split("");
  ...
  useEffect(() => {
    ...
    const onResults = (results) => {
      ...
      if (results.multiFaceLandmarks && results.multiFaceLandmarks[0]) {
       ...
       if (positionIndicator === 'Left' && currentRoute !== '/key14') {
          const timer = setTimeout(() => {
            navigate('/key14');
          }, 2000);
          setGazingLeftTimer(timer);
        } else if (positionIndicator === 'Right' && currentRoute !== '/key15') {
          const timer = setTimeout(() => {
            navigate('/key15');
          }, 2000);
