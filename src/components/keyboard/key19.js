...
const Key19 = () => {
 ...
 // const [averageValue, setAverageValue] = useState(0);
  ...
  const firstBlockKeys = "PQ".split("");
  const secondBlockKeys = "RS".split("");
  ...
  useEffect(() => {
    ...
    const onResults = (results) => {
      ...
      if (results.multiFaceLandmarks && results.multiFaceLandmarks[0]) {
        ...
        if (positionIndicator === 'Left' && currentRoute !== '/key23') {
          const timer = setTimeout(() => {
            navigate('/key23');
          }, 2000);
          setGazingLeftTimer(timer);
        } else if (positionIndicator === 'Right' && currentRoute !== '/key24') {
          const timer = setTimeout(() => {
            navigate('/key24');
          }, 2000);
