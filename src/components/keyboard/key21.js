...
const Key21 = () => {
 ...
 // const [averageValue, setAverageValue] = useState(0);
  ...
  const firstBlockKeys = "WX".split("");
  const secondBlockKeys = "YZ".split("");
  ...
  useEffect(() => {
    ...
    const onResults = (results) => {
      ...
      if (results.multiFaceLandmarks && results.multiFaceLandmarks[0]) {
        ...
        if (positionIndicator === 'Left' && currentRoute !== '/key26') {
          const timer = setTimeout(() => {
            navigate('/key26');
          }, 2000);
          setGazingLeftTimer(timer);
        } else if (positionIndicator === 'Right' && currentRoute !== '/key27') {
          const timer = setTimeout(() => {
            navigate('/key27');
          }, 2000);
