...
const Key4 = () => {
 ...
 // const [averageValue, setAverageValue] = useState(0);
  ...
  const firstBlockKeys = "ABC".split("");
  const secondBlockKeys = "DEF".split("");
  ...
  useEffect(() => {
    ...
    const onResults = (results) => {
      ...
      if (results.multiFaceLandmarks && results.multiFaceLandmarks[0]) {
        ...
        if (positionIndicator === 'Left' && currentRoute !== '/key6') {
          const timer = setTimeout(() => {
            navigate('/key6');
          }, 2000);
          setGazingLeftTimer(timer);
        } else if (positionIndicator === 'Right' && currentRoute !== '/key7') {
          const timer = setTimeout(() => {
            navigate('/key7');
          }, 2000);
