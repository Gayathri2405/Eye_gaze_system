...
const Key6 = () => {
 ...
 // const [averageValue, setAverageValue] = useState(0);
  ...
  const firstBlockKeys = "A".split("");
  const secondBlockKeys = "BC".split("");
  ...
  useEffect(() => {
    ...
    const onResults = (results) => {
      ...
      if (results.multiFaceLandmarks && results.multiFaceLandmarks[0]) {
        ...
        if (positionIndicator === 'Left' && currentRoute !== '/key10') {
          const timer = setTimeout(() => {
            navigate('/key10');
          }, 2000);
          setGazingLeftTimer(timer);
        } else if (positionIndicator === 'Right' && currentRoute !== '/key11') {
          const timer = setTimeout(() => {
            navigate('/key11');
          }, 2000);
