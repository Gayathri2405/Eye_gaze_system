...
const Key16 = () => {
 ...
 // const [averageValue, setAverageValue] = useState(0);
  ...
  const firstBlockKeys = "NO".split("");
  const secondBlockKeys = "PQRS".split("");
  ...
  useEffect(() => {
    ...
    const onResults = (results) => {
      ...
      if (results.multiFaceLandmarks && results.multiFaceLandmarks[0]) {
        ...
        if (positionIndicator === 'Left' && currentRoute !== '/key18') {
          const timer = setTimeout(() => {
            navigate('/key18');
          }, 2000);
          setGazingLeftTimer(timer);
        } else if (positionIndicator === 'Right' && currentRoute !== '/key19') {
          const timer = setTimeout(() => {
            navigate('/key19');
          }, 2000);
