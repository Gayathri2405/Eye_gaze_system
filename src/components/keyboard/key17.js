...
const Key17 = () => {
 ...
 // const [averageValue, setAverageValue] = useState(0);
  ...
  const firstBlockKeys = "TUV".split("");
  const secondBlockKeys = "WXYZ".split("");
  ...
  useEffect(() => {
    ...
    const onResults = (results) => {
      ...
      if (results.multiFaceLandmarks && results.multiFaceLandmarks[0]) {
        ...
        if (positionIndicator === 'Left' && currentRoute !== '/key20') {
          const timer = setTimeout(() => {
            navigate('/key20');
          }, 2000);
          setGazingLeftTimer(timer);
        } else if (positionIndicator === 'Right' && currentRoute !== '/key21') {
          const timer = setTimeout(() => {
            navigate('/key21');
          }, 2000);
