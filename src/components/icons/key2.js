  ...
  useEffect(() => {
    ...
    const onResults = (results) => {
      ...
      if (results.multiFaceLandmarks && results.multiFaceLandmarks[0]) {
        ...
        if (positionIndicator === 'Left' && currentRoute !== '/key4' && currentRoute !== '/key2') {
          const timer = setTimeout(() => {
            navigate('/key4');
          }, 2000);
          setGazingLeftTimer(timer);
        } else if (positionIndicator === 'Right' && currentRoute !== '/key5' && currentRoute !== '/key3') {
          const timer = setTimeout(() => {
            navigate('/key5');
          }, 2000);
  ...
  const Keyboard1 = () => {
    return (
      <div style={{ display: 'flex', justifyContent: 'center', margin: '50px' }}>
        {/* Parent container for the two rectangular boxes */}
        <div style={{ display: 'flex', flexDirection: 'row' }}>
          {/* First rectangular block (A to F) */}
          <div style={{ border: '1px solid black', padding: '10px' }}>
            <button style={{ margin: '5px' }} onClick={handleClearClick}>Clear</button>
            {firstBlock1Keys.map((key, index) => (
              <button key={index} style={{ margin: '5px' }} onClick={() => handleButtonClick(key)}>
                {key}
              </button>
            ))}
          </div>
          {/* Second rectangular block (G to M) */}
          <div style={{ border: '1px solid black', padding: '10px' }}>
            {secondBlock1Keys.map((key, index) => (
              <button key={index} style={{ margin: '5px' }} onClick={() => handleButtonClick(key)}>
                {key}
              </button>
            ))}
